...
find_server() {
    ...
    case "$server_app" in
        ...
        reports)
            ...
            GUNICORN_CMD_ARGS=${GUNICORN_CMD_ARGS:-"--bind=0.0.0.0:8772 --config lib/galaxy/web_stack/gunicorn_config.py"}
            ;;
        coralsnp_reports)
            # TODO: is this really the only way to configure the port?
            GUNICORN_CMD_ARGS=${GUNICORN_CMD_ARGS:-"--bind=0.0.0.0:8770 --config lib/galaxy/web_stack/gunicorn_config.py"}
            ;;
        tool_shed)
