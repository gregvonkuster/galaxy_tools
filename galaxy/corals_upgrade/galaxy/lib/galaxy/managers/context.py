...
class ProvidesAppContext:
    ...
    def expunge_all(self):
        ...
        if hasattr(app, 'corals_model'):
            corals_model = app.corals_model
            if corals_model != app.model:
                corals_model.context.expunge_all()
    ...
    @property
    def corals_model(self) -> ModelMapping:
        """Provide access to Galaxy's corals mapping.

        Comments on the ``model`` property apply here also.
        """
        return self.app.corals_model
