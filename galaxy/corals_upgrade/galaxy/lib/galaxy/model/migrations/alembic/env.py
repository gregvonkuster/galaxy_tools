...
def _run_migrations_invoked_via_script(run_migrations: Callable[[str], None]) -> None:
    ...
    if revision_str.startswith(f"{GXY}@"):
        ...
        url = urls[TSI]
    elif revision_str.startswith(f"{CORALS}@"):
        url = urls[CORALS]
    else:
        ...
        if GXY in revision.branch_labels:
            ...
            url = urls[TSI]
        elif CORALS in revision.branch_labels:
            url = urls[CORALS]
...
def _configure_and_run_migrations_online(url) -> None:
    log.debug("XXX url: %s" % str(url))
    engine = create_engine(url)
    log.debug("XXX engine: %s" % str(engine))
...
def _load_urls() -> Dict[ModelId, str]:
    ...
    corals_url = context_dict.get(f"{CORALS}_url")
