...
CORALS_CONFIG_PREFIX = "CORALS_CONFIG_"
...
class DbScript:
    ...
    def upgrade(self, args: Namespace) -> None:
        ...
        if args.revision:
            ...
            try:
                ...
                self.alembic_config.set_main_option("sqlalchemy.url", self.corals_url)
                upgrade_to_revision("corals@head")
            finally:
                self.alembic_config.set_main_option("sqlalchemy.url", self.gxy_url)
    ...
    def _set_dburl(self, config_file: Optional[str] = None) -> None:
        gxy_config, tsi_config, corals_config, _ = get_configuration_from_file(os.getcwd(), config_file)
        self.gxy_url = gxy_config.url
        self.tsi_url = tsi_config.url
        self.corals_url = corals_config.url
...
class Command:
    ...
    def init(args: Namespace) -> None:
        gxy_config, tsi_config, corals_config, is_auto_migrate = get_configuration(sys.argv, os.getcwd())
        verify_databases_via_script(gxy_config, tsi_config, corals_config, is_auto_migrate)
