...
class MinimalGalaxyApplication(BasicSharedApp, HaltableContainer, SentryClientMixin):
    ...
    def __init__(self, fsmon=False, **kwargs) -> None:
        ...
        self._register_singleton(corals_model_scoped_session, self.corals_model.context)
    ...
    def _configure_engines(self, db_url, install_db_url, corals_db_url, combined_install_database):
        ...
        if not combined_install_database:
            ...
            corals_engine = build_engine(corals_db_url, self.config.corals_database_engine_options)
        return engine, install_engine, corals_engine
    ...
    def _configure_models(self, check_migrate_databases=False, config_file=None):
        ...
        engine, install_engine, corals_engine = self._configure_engines(db_url, install_db_url, corals_db_url, combined_install_database)

        if self.config.database_wait:
            self._wait_for_database(db_url)

        if check_migrate_databases:
            self._verify_databases(engine, install_engine, corals_engine, combined_install_database)
        ...
        self.corals_model = corals_mapping.configure_model_mapping(
            self.config.file_path,
            corals_engine,
            self.config.thread_local_log
        )
        log.info(f"Corals database using its own connection {corals_db_url}")
    ...
    def _verify_databases(self, engine, install_engine, corals_engine, combined_install_database):
        install_template, install_encoding = None, None
        corals_template, corals_encoding = None, None
        if not combined_install_database:  # Otherwise these options are not used.
            ...
            corals_template = getattr(self.config, "corals_database_template", None)
            corals_encoding = getattr(self.config, "corals_database_encoding", None)
