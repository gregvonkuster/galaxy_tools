...
# hopefully the bam file names and vcf file names
...
for i, bam_file in enumerate(bam_files):
    file_name_base = os.path.basename(bam_file)
    ...
    if total_zero_coverage > 0:
        header_file = "%s_header.csv" % file_name_base
        ...
        body_file = "%s_body.csv" % file_name_base
        cat_df.to_csv(body_file, sep='\t', header=False, index=False)
        if args.output_vcf is None:
            output_vcf_file = os.path.join(OUTPUT_VCF_DIR, "%s.vcf" % file_name_base)
        else:
    ...
    bam_metrics = [file_name_base, "", "%4f" % average_coverage, genome_coverage]
    vcf_metrics = [file_name_base, str(good_snp_count), "", ""]
    if args.output_metrics is None:
        output_metrics_file = os.path.join(OUTPUT_METRICS_DIR, "%s.tabular" % file_name_base)
    else:
